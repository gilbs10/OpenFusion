...
__global__ void d_convertDepthToWorldVertices(const CameraData d_cameraData, CAMERA_DEPTH_TYPE* d_newDepth, float* transform, uint* newDepthMap, CameraParams cameraParams) {
	...
	if ((mabs(newDepth) < cameraParams.m_min_depth ||		// TODO: newDepth is possibly non-negative. Maybe this fabs is redundant
		 mabs(newDepth) > cameraParams.m_max_depth)) {
		// No depth at point
